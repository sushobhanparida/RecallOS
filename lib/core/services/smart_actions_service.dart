...
class SmartActionsService {
  ...
  List<SmartAction> actionsFor(List<Screenshot> screenshots, {int limit = 8}) {
    ...
    for (final s in sorted) {
      // 1. Prioritize AI Generated Smart Actions
      if (s.aiAnalysisPayload != null) {
        try {
          final payload = jsonDecode(s.aiAnalysisPayload!);
          if (payload['smart_actions'] is List) {
            for (final aiAction in payload['smart_actions']) {
              final intent = aiAction['intent'] as String?;
              final title = aiAction['title'] as String?;
              final data = aiAction['data'] as String?;
              if (intent != null && title != null && data != null) {
                final action = _aiAction(s, intent, title, data);
                if (action != null) {
                  out.add(action);
      ...
      // 2. Fallback to statically generated actions from Entities
  ...
  SmartAction? _aiAction(Screenshot s, String intent, String title, String data) {
    // Map AI intents to local SmartActionKind if possible
    // Note: To support complex AI actions (like track_package), we'd need to expand SmartActionKind.
    // For now, map basic intents from AI.
    final entity = ExtractedEntity(type: intent, rawText: data);
    ...
    switch (intent) {
      ...
      // Fallback: If AI generates a complex action, we might not have a handler yet.
      default:
        return null;
    }
  }
